...
#define SZ_PTR(Sz) Sz,
const int dft_ftab[] = {FOREACH_DFTSZ(SZ_PTR)};

#define SZ_iPTR(Sz)  Sz,
const int idft_ftab[] = {FOREACH_IDFTSZ(SZ_iPTR)};
...
void armral_dft(uint8_t sizeidx, int16_t *x, int16_t *y, unsigned char scale_flag)
{
  ...
  switch (sizeidx) {
    default:
      ...
      status = armral_fft_create_plan_cs16(&dft_cs16_plan_p, idft_ftab[sizeidx], ARMRAL_FFT_FORWARDS);
...
void armral_idft(uint8_t sizeidx, int16_t *x, int16_t *y, unsigned char scale_flag)
{
  ...
  switch (sizeidx) {
    ...
    default:
      ...
      status = armral_fft_create_plan_cs16(&idft_cs16_plan_p, idft_ftab[sizeidx], ARMRAL_FFT_BACKWARDS);
      AssertFatal(status == ARMRAL_SUCCESS, "Failure in ArmRAL FFT create plan\n");
