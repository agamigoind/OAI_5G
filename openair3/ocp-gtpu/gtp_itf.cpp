...
static gtpv1u_bearer_t create_adhoc_bearer(int socket, uint32_t peerIp, uint16_t peerPort, uint32_t teid, uint16_t seq)
{
  struct sockaddr_in to = {.sin_family = AF_INET, .sin_port = htons(peerPort)};
  memcpy(&to.sin_addr, &peerIp, sizeof(peerIp));
  gtpv1u_bearer_t bearer = {.sock_fd = socket, .teid_outgoing = teid, .seqNum = seq};
  memcpy(&bearer.ip, &to, sizeof(to));
  return bearer;
}
...
int gtpv1LocalIpv4Address(instance_t instance, struct sockaddr_in *sin_out)
{
  ...
  struct sockaddr_in sin = {.sin_family = AF_INET, .sin_port = htons(inst->get_srcport())};
                             ...
                             gtpCallback callBack)
{
  ...
  for (int i = 0; i < create_tunnel_req->num_tunnels; i++) {
    ...
    gtpv1u_bearer_t b;
    int ret = newGtpuCreateTunnel(compatInst(instance),
                                      create_tunnel_req->rnti,
                                      incoming_rb_id,
                                      create_tunnel_req->eps_bearer_id[i],
    ...
    create_tunnel_resp->enb_S1u_teid[i] = b.teid_incoming;
                             ...
                             gtpCallbackSDAP callBackSDAP)
{
  ...
  for (int i = 0; i < create_tunnel_req->num_tunnels; i++) {
    gtpv1u_bearer_t b;
    int ret = newGtpuCreateTunnel(instance,
                                  create_tunnel_req->ue_id,
    ...
    create_tunnel_resp->num_tunnels = create_tunnel_req->num_tunnels;
    create_tunnel_resp->gnb_NGu_teid[i] = b.teid_incoming;
...
static int Gtpv1uHandleEchoReq(int h, uint8_t *msgBuf, uint32_t msgBufLen, uint16_t peerPort, uint32_t peerIp)
{
  ...
  gtpv1u_bearer_t bearer = create_adhoc_bearer(h, peerIp, peerPort, ntohl(msgHdr->teid), seq);
