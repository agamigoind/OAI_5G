...
bool vnf_nr_send_p7_msg(vnf_p7_t *vnf_p7, nfapi_nr_p7_message_header_t *header)
{
  ...
  if (p7_connection) {
    ...
    void *msg_to_send = NULL;
    uint16_t msg_to_send_len = 0;
    if (len > vnf_p7->_public.segment_size) {
      ...
      for (segment = 0; segment < segment_count; ++segment) {
        ...
        msg_to_send = &tx_buffer[0];
        msg_to_send_len = segment_size;
      }
    } else {
      if (vnf_p7->_public.checksum_enabled) {
        nfapi_nr_p7_update_checksum(buffer, len);
      }
      msg_to_send = &buffer[0];
      msg_to_send_len = len;
    }
    uint32_t time =
    ...
    send_result = socket_send_p7_msg(vnf_p7->socket, &(p7_connection->remote_addr), msg_to_send, msg_to_send_len);
