...
bool wls_pnf_nr_send_p7_message(pnf_p7_t *pnf_p7, nfapi_nr_p7_message_header_t *msg, uint32_t msg_len)
{
  ...
  int packed_len = wls_pnf_p7_config->pack_func(msg, pnf->tx_message_buffer, sizeof(pnf->tx_message_buffer), NULL);
    ...
    while (numMsgToGet--) {
      ...
      if (msg_PA != 0) {
        ...
        if (msg->msg_type != FAPI_VENDOR_MSG_HEADER_IND) {
          ...
          procPhyMessages(isRadisysFAPI ? msg->msg_len : len, (void *)(msg + 1), msg->msg_type);
        } else {
          uint8_t *msgt = (uint8_t *)(msg + 1);
