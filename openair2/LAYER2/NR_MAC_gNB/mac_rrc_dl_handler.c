...
void f1_setup_response(const f1ap_setup_resp_t *resp)
{
  ...
  if (update) {
    ...
    prepare_du_configuration_update(mac, info, cc->mib, cc->sib1);
  }
...
void ue_context_setup_request(const f1ap_ue_context_setup_t *req)
{
  ...
  if (mtc) {
    ...
    UE->measGapConfig = get_gap_config_from_smtc(ssb_mtc);
  ...
  free(resp.du_to_cu_rrc_information->measGapConfig);
