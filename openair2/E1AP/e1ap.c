...
{
  e1ap_bearer_context_mod_failure_t msg = {0};
  if (!decode_E1_bearer_context_mod_failure(&msg, pdu)) {
    free_E1_bearer_context_mod_failure(&msg);
    return -1;
  }
  LOG_E(E1AP, "Received Bearer Context Modification Failure from CU-UP %d with cause value %d \n", msg.gNB_cu_up_ue_id, msg.cause.value);
  return 0;
}
