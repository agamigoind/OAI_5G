...
void config_pdcp(configmodule_interface_t *cfg, nr_pdcp_configuration_t *pdcp_config)
{
  ...
  pdcp_config->drb.sn_size = *pdcp_params[CONFIG_NR_PDCP_DRB_SN_SIZE_IDX].iptr;
...
void RCconfig_nr_macrlc(configmodule_interface_t *cfg)
{
  ...
  if (MacRLC_ParamList.numelt > 0) {
    /* NR PDCP config is needed by fill_nr_noS1_bearer_config()
     * called by mac_top_init_gNB()
     */
    nr_pdcp_configuration_t default_pdcp_config;
    config_pdcp(cfg, &default_pdcp_config);

    /* NR RLC config is needed by fill_nr_noS1_bearer_config()
     * called by mac_top_init_gNB()
     */
    ...
    mac_top_init_gNB(node_type, scc, scd, &config, &default_pdcp_config, &default_rlc_config);
