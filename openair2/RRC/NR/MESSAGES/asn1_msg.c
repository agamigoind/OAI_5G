...
static NR_RRCReconfiguration_IEs_t *build_RRCReconfiguration_IEs(const nr_rrc_reconfig_param_t *params)
{
  ...
  /* secondaryCellGroup:
   * - Mandatory: RRCReconfiguration message (carried by RRCResume or RRCConnectionReconfiguration)
   * - Optional: RRCReconfiguration message on SRB3 or another RRCReconfiguration message
   * - Absent otherwise */
  ...
  if (ie->nonCriticalExtension) {
    ...
    /* fullConfig:
     * - Mandatory present: inter-system handover from E-UTRA/EPC to NR
     * - Optionally present: (1) Reconfiguration with synchronization
     *   (2) First reconfiguration after reestablishment
     *   (3) Intra-system handover from E-UTRA/5GC to NR
     * - Absent otherwise */
...
byte_array_t do_RRCReconfiguration(const nr_rrc_reconfig_param_t *params)
{
  ...
  msg.len = uper_encode_to_new_buffer(&asn_DEF_NR_DL_DCCH_Message, NULL, &dl_dcch_msg, (void **)&msg.buf);
  ...
  // set fields with pointers to NULL, is freed in the context of memory allocation
  ie->measConfig = NULL;
